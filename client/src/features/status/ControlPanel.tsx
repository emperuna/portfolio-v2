...
interface ControlPanelProps {
    ...
    trafficLevel: 'low' | 'high' | 'surge';
    setTrafficLevel: (val: 'low' | 'high' | 'surge') => void;
}

export function ControlPanel({ debugMode, setDebugMode, trafficLevel, setTrafficLevel }: ControlPanelProps) {
    const cycleTraffic = () => {
        if (trafficLevel === 'low') setTrafficLevel('high');
        else if (trafficLevel === 'high') setTrafficLevel('surge');
    ...
    return (
        <div className="h-[150px] border-t border-primary/20 p-4 font-mono text-xs flex flex-col justify-between bg-primary/5 select-none">
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between group cursor-pointer" onClick={cycleTraffic}>
                    ...
                    <div className="flex items-center space-x-2 text-[9px] bg-black/20 rounded px-1 py-0.5">
                        ...
                        <span className="text-primary/20">|</span>
                        <span className={trafficLevel === 'surge' ? 'text-red-400 animate-pulse' : 'text-primary/30'}>SURGE</span>
                    </div>
                </div>
