                    ...
                    <div className="grid grid-cols-3 gap-2 mt-1">
                        {(['low', 'high', 'surge'] as const).map((level) => (
                            <button
                                ...
                                className={`py-1.5 text-[10px] uppercase font-bold rounded-sm border transition-all ${trafficLevel === level ? 'bg-cyan-500/10 border-cyan-500/50 text-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.2)]' : 'bg-[#0a0a0c] border-white/10 text-slate-500 hover:border-white/20 hover:text-white'}`}
                            >
                                {level}
                            </button>
                        ))}
                        ...
                        {(['standard', 'maintenance', 'offline'] as const).map((mode) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`py-1.5 text-[10px] uppercase font-bold rounded-sm border transition-all ${simMode === mode ? activeColor : 'bg-[#0a0a0c] border-white/10 text-slate-500 hover:border-white/20 hover:text-white'}`}
                                >
                                    {mode}
                                </button>
                            );
